...
/*
 * Description:
 *   This file implements the Secure Hash Algorithm SHA-256
 ...
 *
 *   The SHA-256 algorithm produces a 256-bit message digests for
 *   a given data stream.  It should take about 2**n steps to find
 *   a message with the same digest as a given message and 2**(n/2)
 *   to find any two messages with the same digest, when n is the
 *   digest size in bits.  Therefore, this algorithm can serve as a
 *   means of providing a "fingerprint" for a message.
 *
 * Portability Issues:
 *   SHA-256 is defined in terms of 32-bit "words".
 ...
 *
 * Caveats:
 *   SHA-256 is designed to work with messages less than 2^64 bits long.
 *   This implementation uses 256Input() to hash the bits that are a
 *   multiple of the size of an 8-bit octet, and then optionally uses 
...
static void SHA256Finalize(SHA256Context *context,
                           uint8_t Pad_Byte);

static void SHA256PadMessage(SHA256Context *context,
                             uint8_t Pad_Byte);
                    ...
                    uint8_t message_bits, unsigned int length)
{
    ...
    SHA256Finalize(context, (uint8_t)
 ...
 *   preparation for computing a new SHA-256 message digest.
 *
 * Parameters:
 *   context: [in/out]
